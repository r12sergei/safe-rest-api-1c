...
Функция КонтекстЗапроса(Запрос, ОбеспечениеИдемпотентности = Ложь, КонтрольКонфликтовЗапросов = Ложь,
	...
	Если Контекст.НастройкиЛогированияПоШаблонам.ИспользоватьНастройкиЛогированияПоШаблонам
		И Не Контекст.ПроверитьИдемпотентностьЗапроса Тогда
		
		Если Контекст.НастройкиЛогированияПоШаблонам.СпособЛогирования = Перечисления.кв_СпособыЛогирования.НеЛогировать
			Или Не ЗначениеЗаполнено(Контекст.НастройкиЛогированияПоШаблонам.Шаблон) Тогда
			ЗаписыватьЛог = Ложь;
		КонецЕсли;
...
Функция ОтветНаЗапрос(Контекст, ПреобразоватьДанные = Истина) Экспорт
	...
	ДополнитьКонтекстЗапросаПослеОбработки(Контекст, Ответ);
...
// Возвращает результат проверки, что код состояния определяет успешное выполнение метода.
// 
// Параметры:
//  КодСостояния - Число - код состяния.
// 
// Возвращаемое значение:
//  Булево - признак, что это код успешного выполнения.
//
Функция ЭтоКодУспешногоВыполнения(КодСостояния) Экспорт
...
Функция КонструкторКонтекстаЗапроса(Запрос, ОбеспечениеИдемпотентности, КонтрольКонфликтовЗапросов)
	...
	НастройкиЛогированияПоШаблонам = НовыйНастройкиЛогированияПоШаблонам();
	
	Контекст.Вставить("ОчищатьОтветВЛоге", Ложь);
	
	Если НастройкиСервиса.ИспользоватьНастройкиЛогированияПоШаблонам Тогда
		
		НастройкиЛогированияПоШаблонам.ИспользоватьНастройкиЛогированияПоШаблонам = Истина;
		
		ДанныеШаблонаСервисов = Справочники.кв_ШаблоныСервисов.ДанныеШаблонаСервисов(
			НастройкиСервиса.Сервис,
			Справочники.кв_ШаблоныСервисов.ВосстановитьИмяШаблонаПоПараметрам(Запрос.ОтносительныйURL, Запрос.ПараметрыURL),
			Метод);
		
		ЗаполнитьЗначенияСвойств(НастройкиЛогированияПоШаблонам, ДанныеШаблонаСервисов);
		
		Если Не ЗначениеЗаполнено(НастройкиЛогированияПоШаблонам.СпособЛогирования) Тогда
			НастройкиЛогированияПоШаблонам.СпособЛогирования = Перечисления.кв_СпособыЛогирования.СпособЛогированияПоУмолчанию();
		КонецЕсли;
		
		Контекст.ОчищатьОтветВЛоге = ДанныеШаблонаСервисов.ОчищатьОтветВЛоге;
		
	КонецЕсли;
	
	Контекст.Вставить("НастройкиЛогированияПоШаблонам", НастройкиЛогированияПоШаблонам);	
...
// Возвращает не заполненную структуру настроек логирования по шаблонам.
//
// Возвращаемое значение:
//  Структура - Структура настроек.
//
Функция НовыйНастройкиЛогированияПоШаблонам()
	
	Результат = Новый Структура;
	Результат.Вставить("ИспользоватьНастройкиЛогированияПоШаблонам", Ложь);
...
Процедура ДополнитьКонтекстЗапросаПослеОбработки(Контекст, Ответ)
	
	МедиаТип_ДвоичныеДанныеБезУказанияФормата = МедиаТип_ДвоичныеДанныеБезУказанияФормата();
	
	ТипКонтента = Ответ.Заголовки.Получить("Content-type");
	
	Если ТипКонтента <> Неопределено И ТипКонтента = МедиаТип_ДвоичныеДанныеБезУказанияФормата Тогда
		Контекст.ОчищатьОтветВЛоге = Истина;
	КонецЕсли;
		
КонецПроцедуры

Функция МедиаТип_ДвоичныеДанныеБезУказанияФормата() Экспорт
