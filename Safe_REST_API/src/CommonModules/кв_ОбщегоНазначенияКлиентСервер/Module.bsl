...
Функция ЧастиСтрокиURL(ОбрабатываемаяСтрока) Экспорт
	
	ЧастиСтроки = СтрРазделить(ОбрабатываемаяСтрока, СимволРазделителяПараметровСтрокиURL());
	
	Если ЧастиСтроки.Количество() > 0 И ПустаяСтрока(ЧастиСтроки[0]) Тогда
...
Функция СимволРазделителяПараметровСтрокиURL() Экспорт
	
	СимволРазделителяПараметров = "/";
	Возврат СимволРазделителяПараметров;
	
КонецФункции

Процедура ОтформатироватьНаименованиеШаблонаСервиса(СтрокаURL) Экспорт
	
	СтрокаURL = НРег(СокрЛП(СтрокаURL));
	УдалитьЗакрывающийСимволРазделителяВСтрокеURL(СтрокаURL);
...
Процедура УдалитьЗакрывающийСимволРазделителяВСтрокеURL(СтрокаURL)
	
	Если ПустаяСтрока(СтрокаURL) Тогда
		Возврат;
	КонецЕсли;
	
	Если Прав(СтрокаURL, 1) = СимволРазделителяПараметровСтрокиURL() Тогда
		СтрокаURL = Лев(СтрокаURL, СтрДлина(СтрокаURL) - 1);
	КонецЕсли;
