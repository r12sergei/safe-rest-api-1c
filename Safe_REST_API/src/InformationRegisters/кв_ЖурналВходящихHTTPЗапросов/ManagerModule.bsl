...
Процедура ЗафиксироватьПараметрыЗапроса(Контекст) Экспорт
	...
	МенеджерЗаписи.ПараметрыЗапроса = СокрЛП(ПредставлениеПараметровЗапроса(Контекст.ПараметрыЗапросаИзURL));
	
	Если Контекст.Свойство("НастройкиЛогированияПоШаблонам") Тогда
		Если Контекст.НастройкиЛогированияПоШаблонам.ИспользоватьНастройкиЛогированияПоШаблонам Тогда
			МенеджерЗаписи.Шаблон = Контекст.НастройкиЛогированияПоШаблонам.Шаблон;
		КонецЕсли;
	КонецЕсли;	
...
Процедура ЗафиксироватьОтвет(Контекст, Ответ) Экспорт 
	...
	Попытка
		...
		ЭтоКодУспешногоВыполнения = кв_HTTPСервисы.ЭтоКодУспешногоВыполнения(Ответ.КодСостояния);
		...
		Если Контекст.НастройкиЛогированияПоШаблонам.ИспользоватьНастройкиЛогированияПоШаблонам
			...
			СпособЛогирования = Контекст.НастройкиЛогированияПоШаблонам.СпособЛогирования;
			
			Если СпособЛогирования = Перечисления.кв_СпособыЛогирования.НеЛогировать
				Или (СпособЛогирования = Перечисления.кв_СпособыЛогирования.ТолькоОшибки
					И ЭтоКодУспешногоВыполнения)
		...
		Если Контекст.ОчищатьОтветВЛоге
			И ЭтоКодУспешногоВыполнения
			И НЕ Контекст.ПроверитьИдемпотентностьЗапроса Тогда
			ПредставлениеОтвета = "";
		КонецЕсли;
